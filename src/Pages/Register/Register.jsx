...
// كومبوننت المعلومات الثابتة اللي على الشمال
function AuthInfo() {
  const features = [
    {
      icon: "🎓",
...
export default function Register() {
  ...
  async function handleRegister(values) {
    ...
    const payload = {
      ...
      // الـ API doc لم يطلب رقم الجوال في هذه الخطوة
      // phone: values.phone
    };
    ...
    try {
      ...
      // ممكن نوديه لصفحة تفعيل الإيميل أو الهوم
      navigate("/");
    } catch (error) {
      console.error("Register Error:", error);
      if (error.response && error.response.data) {
        // إذا الباك إند بعت أخطاء validation
  ...
  // 2. قواعد التحقق (Validation) - (زي ما هي عشان تطابق الديزاين)
  ...
  const formik = useFormik({
    initialValues: {
      ...
      lastName: "",
      email: "",
      phone: "", // ما زال موجوداً في الفورم
      password: "",
      rePassword: "",
      agreeToTerms: false,
    },
